...
def monte_carlo_sample(env, pi, gamma=0.9, debug=False):
    done = False
    s = env.reset()
    G = []
    t = 0
...
def monte_carlo(env, x, w, update_policy, update_weights, nE, gamma=0.9, alpha=0.01, debug=False):
    w_trained = np.copy(w)
    for e in range(nE):
        if e % 1000 == 0:
        ...
        pi = update_policy(x, w_trained, env.action_space.n,
                           e, nE, debug=debug)
        G = monte_carlo_sample(env, pi, gamma, debug=debug)
